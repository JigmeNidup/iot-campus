...
static bool shouldEnterProvisioningByTripleReset() {
  ...
  // Flash-backed triple reset detector:
  // - Increment a counter at boot.
  // - If the device stays up >3s, clear the counter.
  // - If you reset quickly, the counter persists and reaches 3.
  ...
  if (resetBootCount >= 3) {
    // Clear immediately so a reset during AP mode returns to normal behavior.
    prefs.begin("rst", false);
