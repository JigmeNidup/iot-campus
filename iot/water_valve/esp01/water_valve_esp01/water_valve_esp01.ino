...
static const char* MQTT_CLIENT_ID_PREFIX = "esp01-valve-";

// ESP-01 typically uses GPIO0 or GPIO2 for a relay/valve driver.
// Keep default aligned with ESP-01 light sketch behavior.
...
void setValve(bool on) {
  valveState = on;
  if (VALVE_ACTIVE_LOW) digitalWrite(VALVE_PIN, on ? LOW : HIGH);
  else digitalWrite(VALVE_PIN, on ? HIGH : LOW);
}
...
void startPortal() {
  ...
  WiFi.disconnect();
  WiFi.mode(WIFI_AP);
  WiFi.softAP("ESP01ValveSetup");
  portal.on("/", HTTP_GET, []() {
    portal.send(
      200,
      "text/html",
      "<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'/>"
      "<title>ESP-01 Water Valve Setup</title>"
      "<style>"
      ...
      "</style></head><body>"
      "<div class='card'>"
      "<h2>ESP-01 Water Valve Setup</h2>"
...
void performOta(const String& url, const String& newVersion) {
  ...
  if (url.startsWith("https://")) {
    ...
    if (ret == HTTP_UPDATE_OK && newVersion.length() > 0) {
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (t == otaTopic()) {
    ...
    int end = body.indexOf("\"", start);
    if (end <= start) return;
    String otaUrl = body.substring(start, end);
